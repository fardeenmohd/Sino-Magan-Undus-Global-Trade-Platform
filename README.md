...
- **🧹 Data Clean Slate & Purge Utilities**: 1-click **`🧹 Reset Data Slate`** button on Admin Portal to purge all mock data caches and start with clean, authentic export goods and verified leads.
- **🌐 Multi-Source SME Scraper Engine**: Web scraper crawler queries beyond government portals across private B2B platforms (`Europages.com`, `Kompass.com`), regional chambers of commerce (`tokyochamber.or.jp`, `chamber.de`, `stockholmchamber.se`), and corporate SME registries (`kvk.nl`, `bolagsverket.se`, `krs-online.pl`). Discovers authentic small & medium enterprises (SMEs), boutique organic shops, and specialty importers alongside enterprise syndicates.
- **🌍 Country Corridor Pages (`/countries/[slug]`) & Exporter Profiles (`/exporters/[id]`)**: Full dedicated trade corridor pages for target destination countries (Japan 🇯🇵, Germany 🇩🇪, Sweden 🇸🇪, Poland 🇵🇱, Netherlands 🇳🇱, Australia 🇦🇺, Oman 🇴🇲, UAE 🇦🇪) displaying regulatory tariff overviews, connected Indian products, and verified buyer prospects. Dedicated Exporter Profile pages showcasing IEC registration, APEDA/FSSAI audit badges, processing facilities, connected product portfolios, and 1-click FOB RFQ quote requests.
